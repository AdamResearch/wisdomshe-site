...
SESSION_COOKIE = 'wisdomshe_admin_token'
SESSION_TOKEN = None
        ...
        samples = [
            {
                ...
                'service_interest': '战略诊断',
                'project_stage': '初步了解',
                'current_issue': '希望先看项目方向是否匹配',
                'status': 'new',
            },
            {
                'kind': 'membership',
                ...
                'business_line': 'capital',
                'source_page': '/portfolio/',
                'service_interest': '组合结构设计',
                'project_stage': '已沟通',
                'current_issue': '需要查看组合管理的需求入口',
                'status': 'reviewing',
            },
        ]
        for index, sample in enumerate(samples, start=1):
            ...
            conn.execute(
                ...
                (
                    ...
                    sample.get('phone'),
                    sample.get('email'),
                    sample.get('business_line'),
                    sample.get('source_page'),
                    sample.get('service_interest'),
                    sample.get('project_stage'),
                    sample.get('current_issue'),
                    sample.get('status', 'new'),
...
def is_admin_logged_in() -> bool:
    return SESSION_TOKEN is not None and get_token() == SESSION_TOKEN
...
def create_application():
    ...
    timestamp = now_iso()
    kind = str(payload.get('kind', 'diagnostic')).strip() or 'diagnostic'
    with db() as conn:
        ...
        conn.execute(
            ...
            (
                ...
                str(payload.get('business_line', 'general')).strip(),
                str(payload.get('source_page', '')).strip(),
                str(payload.get('service_interest', '')).strip(),
                str(payload.get('project_stage', '')).strip(),
                str(payload.get('current_issue', '')).strip(),
                'new',
